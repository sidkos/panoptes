...
class PrometheusSource:
    ...
    def health(self) -> SourceHealth:
        """Probe reachability via a cheap `GET /-/healthy`, catching any error (no raise).

        Prometheus exposes `/-/healthy` as a trivial liveness endpoint. An unreachable
        endpoint (connection refused, timeout, a non-2xx) is CAUGHT and surfaced as
        `reachable=False` — it does NOT propagate, so the collector's per-source try/continue
        boundary keeps the rest of the cycle running. The `detail` is a GENERIC summary
        (exception class only), never a verbatim `str(exc)` that could echo a token/endpoint
        through the MCP-visible `describe_health` rollup (F4, the sentry/cloudwatch discipline).
        """
        checked_at = datetime.now(UTC)
        try:
            # `/-/healthy` returns plain text ("Prometheus is Healthy."), not JSON — so use
            # the raw `send` (raise_for_status + body-surfacing) rather than `get_json`
            # (which would try to `.json()`-decode the text body and fail spuriously).
            health_url = f"{self._url}/-/healthy"
            self._rest.send(
                lambda http: http.get(health_url),
            ...
            return SourceHealth(
                reachable=False,
                detail=(
                    f"prometheus endpoint unreachable "
        ...
        return SourceHealth(
            reachable=True,
            detail=f"prometheus endpoint reachable ({self._url})",
            checked_at=checked_at,
        )
